...
\begin{questions}
    ...
    \begin{parts}
        ...
        \begin{solution}
            Wir wissen, dass eine Matrix $A \in \C^{n\times n}$ genau dann invertierbar ist, wenn $\det(A) \neq 0$. 

            Seien $\lambda_i$ die Eigenwerte von $A$. 
            Damit gilt: 
            $$
                \det(A) \neq 0 \quad \land \quad \det(A) = \prod_{i \in \interval{1, n}} \lambda_i \quad \iff \quad \forall i \in \interval{1, n} : \lambda_i \neq 0
            $$\qed
        \end{solution}
        ...
        Gilt die Umkehrung?
        \begin{solution}
            Die gegebene Aussage gilt bereits nicht. 
            So ist z.B. $\vektor{i & 0 \\ 0 & 2i}$ symmetrisch, hat aber komplexe Eigenwerte. 

            Vermutlich wurde aber von einer reellsymmetrischen Matrix $A$ ausgegangen. 

            Sei $\lambda$ ein Eigenwert mit Eigenvektor $v$ von $A$. 
            Dann gilt: 
            $$
            \lambda \scalarprod{v, v} = \scalarprod{\lambda v, v} = \scalarprod{A v, v} = (Av)^T \bar{v} = v^TA^T\bar{v} = v^TA\bar{v} = \scalarprod{v, \bar{A}v} = \scalarprod{v, Av} = \scalarprod{v, \lambda v} = \bar{\lambda} \scalarprod{v, v}
            $$

            Und damit 
            $$
            \lambda = \bar{\lambda} \quad \implies \quad \lambda \in \R
            $$\qed

            Die Rückrichtung gilt nicht. 

            Ein Gegenbeispiel ist die nicht-symmetrische Matrix $\vektor{1 & 1 \\ 0 & 1}$ mit dem reellen Eigenwert $1$. \qed
        \end{solution}
    \end{parts}
\end{questions}
\end{document}